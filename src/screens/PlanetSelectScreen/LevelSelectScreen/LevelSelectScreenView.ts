...
export class LevelSelectScreenView {
    ...
    constructor(onLevelSelect: (level: number) => void, onBackClick: () => void, onShopClick: () => void, planetType: planetName) {
        ...
        if (planetType === "tutorial_planet") {
            this.view = new TutorialLevelSelectView(onLevelSelect, onBackClick, onShopClick, planetType);
        } else {
            this.view = new CampaignLevelSelectView(onLevelSelect, onBackClick, onShopClick, planetType);
        }
    }
